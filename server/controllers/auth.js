    ...
    login: async (req, res) => {        
        try{
            ...
            if(foundUser) {
                ...
                const isAuthenticated = bcrypt.compare(password, foundUser.dataValues.user_hashPass);
                console.log(isAuthenticated);
                // will check if isAuthenticated is truthy.
                if(isAuthenticated) {
                    // this makes a new token for our new user passing in the user_email and id
                    let token = createToken(foundUser.dataValues.user_email, foundUser.dataValues.userId);
                    ...
                    const userData = {
                    user_email: foundUser.dataValues.user_email,
